...
The TUI defaults to `RUST_LOG=kardashev_core=info,kardashev_tui=info,kardashev_rmcp_client=info` and log messages are written to `~/.kardashev/log/kardashev-tui.log`, so you can leave the following running in a separate terminal to monitor log messages as they are written:

```bash
tail -F ~/.kardashev/log/kardashev-tui.log
```
...
| `config`                | object | Individual [config settings](https://github.com/openai/codex/blob/main/docs/config.md#config) that will override what is in `$KARDASHEV_HOME/config.toml`. |
