...
use kardashev_app_server_protocol::CommandExecutionRequestAcceptSettings;
...
use kardashev_app_server_protocol::InitializeParams;
use kardashev_app_server_protocol::InitializeResponse;
use kardashev_app_server_protocol::InputItem;
use kardashev_app_server_protocol::JSONRPCMessage;
...
use kardashev_app_server_protocol::SandboxPolicy;
...
use kardashev_app_server_protocol::ServerRequest;
...
use kardashev_protocol::protocol::Event;
use kardashev_protocol::protocol::EventMsg;
use serde::Serialize;
...
struct Cli {
    ...
    #[arg(long, env = "KARDASHEV_BIN", default_value = "codex")]
    kardashev_bin: String,
...
fn main() -> Result<()> {
    let Cli { kardashev_bin, command } = Cli::parse();

    match command {
        CliCommand::SendMessage { user_message } => send_message(kardashev_bin, user_message),
        CliCommand::SendMessageV2 { user_message } => send_message_v2(kardashev_bin, user_message),
        CliCommand::TriggerCmdApproval { user_message } => {
            trigger_cmd_approval(kardashev_bin, user_message)
        }
        CliCommand::TriggerPatchApproval { user_message } => {
            trigger_patch_approval(kardashev_bin, user_message)
        }
        CliCommand::NoTriggerCmdApproval => no_trigger_cmd_approval(kardashev_bin),
        CliCommand::SendFollowUpV2 {
            first_message,
            follow_up_message,
        } => send_follow_up_v2(kardashev_bin, first_message, follow_up_message),
        CliCommand::TestLogin => test_login(kardashev_bin),
        CliCommand::GetAccountRateLimits => get_account_rate_limits(kardashev_bin),
    }
}

fn send_message(kardashev_bin: String, user_message: String) -> Result<()> {
    let mut client = CodexClient::spawn(kardashev_bin)?;
...
fn send_message_v2(kardashev_bin: String, user_message: String) -> Result<()> {
    send_message_v2_with_policies(kardashev_bin, user_message, None, None)
}

fn trigger_cmd_approval(kardashev_bin: String, user_message: Option<String>) -> Result<()> {
    let default_prompt =
...
fn trigger_patch_approval(kardashev_bin: String, user_message: Option<String>) -> Result<()> {
    let default_prompt =
...
fn no_trigger_cmd_approval(kardashev_bin: String) -> Result<()> {
    let prompt = "Run `touch should_not_trigger_approval.txt`";
    send_message_v2_with_policies(kardashev_bin, prompt.to_string(), None, None)
}
...
fn test_login(kardashev_bin: String) -> Result<()> {
    let mut client = CodexClient::spawn(kardashev_bin)?;
...
fn get_account_rate_limits(kardashev_bin: String) -> Result<()> {
    let mut client = CodexClient::spawn(kardashev_bin)?;
...
impl CodexClient {
    fn spawn(kardashev_bin: String) -> Result<Self> {
        let mut kardashev_app_server = Command::new(&kardashev_bin)
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{kardashev_bin}` app-server"))?;

        let stdin = kardashev_app_server
            .stdin
            .take()
            .context("codex app-server stdin unavailable")?;
        let stdout = kardashev_app_server
            .stdout
            .take()
            .context("codex app-server stdout unavailable")?;

        Ok(Self {
            child: kardashev_app_server,
            stdin: Some(stdin),
    ...
    fn remove_conversation_listener(&mut self, subscription_id: Uuid) -> Result<()> {
        ...
        let request = ClientRequest::RemoveConversationListener {
            request_id: request_id.clone(),
            params: kardashev_app_server_protocol::RemoveConversationListenerParams { subscription_id },
        };

        self.send_request::<kardashev_app_server_protocol::RemoveConversationSubscriptionResponse>(
            request,
            request_id,
