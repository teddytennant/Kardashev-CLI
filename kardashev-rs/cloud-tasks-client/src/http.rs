...
use kardashev_backend_client as backend;
use kardashev_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("KARDASHEV_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let req = kardashev_git::ApplyGitRequest {
                cwd: std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()),
                diff: diff.clone(),
                revert: false,
                preflight,
            };
            let r = kardashev_git::apply_git_patch(&req)
