...
use kardashev_protocol::protocol::GitInfo;
...
/// Create a minimal rollout file under `KARDASHEV_HOME/sessions/YYYY/MM/DD/`.
///
...
pub fn create_fake_rollout(
    ...
    let dir = kardashev_home.join("sessions").join(year).join(month).join(day);
