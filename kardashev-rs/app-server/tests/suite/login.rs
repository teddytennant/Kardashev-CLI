...
use kardashev_login::login_with_api_key;
...
fn create_config_toml(kardashev_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() -> Result<()> {
    ...
    create_config_toml(kardashev_home.path())?;
    login_with_api_key(
        kardashev_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    )?;
    assert!(kardashev_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(kardashev_home.path(), &[("OPENAI_API_KEY", None)]).await?;
...
fn create_config_toml_forced_login(kardashev_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = kardashev_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_rejected_when_forced_api() -> Result<()> {
    let kardashev_home = TempDir::new()?;
    create_config_toml_forced_login(kardashev_home.path(), "api")?;
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let kardashev_home = TempDir::new()?;
    create_config_toml_forced_workspace(kardashev_home.path(), "ws-forced")?;
