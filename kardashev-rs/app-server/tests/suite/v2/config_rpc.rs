...
use kardashev_app_server_protocol::ConfigEdit;
...
fn write_config(kardashev_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
        kardashev_home.path().join("config.toml"),
        contents,
    )?)
}
...
"#,
    ...
    let mut mcp = McpProcess::new_with_env(
        kardashev_home.path(),
        &[("KARDASHEV_MANAGED_CONFIG_PATH", Some(&managed_path_str))],
    )
    .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
"#,
    ...
    let expected_file_path = kardashev_home
        .path()
        .join("config.toml")
        .canonicalize()
        .unwrap()
...
"#,
    ...
    let write_id = mcp
        .send_config_value_write_request(ConfigValueWriteParams {
            file_path: Some(kardashev_home.path().join("config.toml").display().to_string()),
            key_path: "model".to_string(),
            value: json!("gpt-new"),
            merge_strategy: MergeStrategy::Replace,
