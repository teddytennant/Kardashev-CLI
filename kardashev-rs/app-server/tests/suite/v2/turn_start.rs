...
use kardashev_app_server_protocol::PatchChangeKind;
...
use kardashev_core::protocol_config_types::ReasoningSummary;
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    create_config_toml(kardashev_home.path(), &server.uri(), "never")?;

    let mut mcp = McpProcess::new(kardashev_home.path()).await?;
    ...
    assert_eq!(
        started.turn.status,
        kardashev_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    ...
    let image_path = kardashev_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let kardashev_home = tmp.path().to_path_buf();
    ...
    create_config_toml(kardashev_home.as_path(), &server.uri(), "untrusted")?;

    let mut mcp = McpProcess::new(kardashev_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": kardashev_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(kardashev_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(kardashev_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let kardashev_home = tmp.path().join("kardashev_home");
    std::fs::create_dir(&kardashev_home)?;
    ...
    let server = create_mock_chat_completions_server(responses).await;
    create_config_toml(&kardashev_home, &server.uri(), "untrusted")?;

    let mut mcp = McpProcess::new(&kardashev_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(kardashev_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(kardashev_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![kardashev_app_server_protocol::FileUpdateChange {
...
async fn command_execution_notifications_include_process_id() -> Result<()> {
    ...
    let kardashev_home = TempDir::new()?;
    create_config_toml(kardashev_home.path(), &server.uri(), "never")?;
    let config_toml = kardashev_home.path().join("config.toml");
...
fn create_config_toml(
    ...
    let config_toml = kardashev_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
