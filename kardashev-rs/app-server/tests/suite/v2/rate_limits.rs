...
use kardashev_app_server_protocol::RateLimitSnapshot;
use kardashev_app_server_protocol::RateLimitWindow;
...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(kardashev_home.path(), &server_url)?;
...
fn write_chatgpt_base_url(kardashev_home: &Path, base_url: &str) -> std::io::Result<()> {
