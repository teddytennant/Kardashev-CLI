...
use kardashev_core::spawn::KARDASHEV_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
async fn test_kardashev_jsonrpc_conversation_flow() -> Result<()> {
    if env::var(KARDASHEV_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
    ...
    create_config_toml(&kardashev_home, &server.uri())?;
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![kardashev_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await?;
...
fn create_config_toml(kardashev_home: &Path, server_uri: &str) -> std::io::Result<()> {
