...
use kardashev_app_server_protocol::ConfigLayer;
use kardashev_app_server_protocol::ConfigLayerMetadata;
use kardashev_app_server_protocol::ConfigLayerName;
use kardashev_app_server_protocol::ConfigReadParams;
use kardashev_app_server_protocol::ConfigReadResponse;
use kardashev_app_server_protocol::ConfigValueWriteParams;
use kardashev_app_server_protocol::ConfigWriteErrorCode;
use kardashev_app_server_protocol::ConfigWriteResponse;
use kardashev_app_server_protocol::JSONRPCErrorError;
use kardashev_app_server_protocol::MergeStrategy;
use kardashev_app_server_protocol::OverriddenMetadata;
use kardashev_app_server_protocol::WriteStatus;
use kardashev_core::config::ConfigToml;
use kardashev_core::config_loader::LoadedConfigLayers;
use kardashev_core::config_loader::LoaderOverrides;
use kardashev_core::config_loader::load_config_layers_with_overrides;
use kardashev_core::config_loader::merge_toml_values;
...
const MDM_SOURCE: &str = "com.openai.kardashev/config_toml_base64";
...
impl ConfigApi {
    pub(crate) fn new(kardashev_home: PathBuf, cli_overrides: Vec<(String, TomlValue)>) -> Self {
        Self {
            kardashev_home,
            cli_overrides,
    ...
    async fn apply_edits(
        ...
        let allowed_path = self.kardashev_home.join(CONFIG_FILE_NAME);
    ...
    async fn load_layers_state(&self) -> std::io::Result<LayersState> {
        let LoadedConfigLayers {
            ...
            managed_config,
            managed_preferences,
        } = load_config_layers_with_overrides(&self.kardashev_home, self.loader_overrides.clone())
            .await?;

        let user = LayerState::new(
            ConfigLayerName::User,
            self.kardashev_home.join(CONFIG_FILE_NAME),
            base,
        );
        ...
        let system = managed_config.map(|cfg| {
            LayerState::new(
                ...
                system_config_path(&self.kardashev_home),
                cfg,
            )
        });
    ...
    async fn persist_user_config(&self, user_config: &TomlValue) -> anyhow::Result<()> {
        let kardashev_home = self.kardashev_home.clone();
        let serialized = toml::to_string_pretty(user_config)?;

        task::spawn_blocking(move || -> anyhow::Result<()> {
            std::fs::create_dir_all(&kardashev_home)?;

            let target = kardashev_home.join(CONFIG_FILE_NAME);
            let tmp = NamedTempFile::new_in(&kardashev_home)?;
...
fn system_config_path(kardashev_home: &Path) -> PathBuf {
    if let Ok(path) = std::env::var("KARDASHEV_MANAGED_CONFIG_PATH") {
        return PathBuf::from(path);
    }

    #[cfg(unix)]
    {
        let _ = kardashev_home;
        PathBuf::from("/etc/codex/managed_config.toml")
    }

    #[cfg(not(unix))]
    {
        kardashev_home.join("managed_config.toml")
    }
}
