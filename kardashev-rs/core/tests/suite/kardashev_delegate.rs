...
async fn kardashev_delegate_forwards_exec_approval_and_proceeds_on_approval() {
    skip_if_no_network!();
    ...
    wait_for_event(&test.kardashev, |ev| {
    ...
    .await;
    wait_for_event(&test.kardashev, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
}

/// Delegate should surface ApplyPatchApprovalRequest and honor parent decision
/// so the sub-agent can proceed to completion.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn kardashev_delegate_forwards_patch_approval_and_proceeds_on_decision() {
    skip_if_no_network!();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn kardashev_delegate_ignores_legacy_deltas() {
    skip_if_no_network!();
    ...
    loop {
        let ev = wait_for_event(&test.kardashev, |_| true).await;
        match ev {
