...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.kardashev, |ev| {
    ...
    let end_event = wait_for_event(&fixture.kardashev, |ev| {
    ...
    wait_for_event(&fixture.kardashev, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
...
async fn stdio_image_completions_round_trip() -> anyhow::Result<()> {
    ...
    let fixture = test_codex()
        .with_config(move |config| {
            config.model_provider.wire_api = kardashev_core::WireApi::Chat;
            config.features.enable(Feature::RmcpClient);
            config.mcp_servers.insert(
                server_name.to_string(),
                McpServerConfig {
...
/// This test writes to a fallback credentials file in KARDASHEV_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire KARDASHEV_HOME through the code.
#[serial(kardashev_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("KARDASHEV_HOME", temp_home.path().as_os_str());
