...
fn write_minimal_rollout_with_id(kardashev_home: &Path, id: Uuid) -> PathBuf {
    let sessions = kardashev_home.join("sessions/2024/01/01");
...
#[tokio::test]
async fn find_handles_gitignore_covering_kardashev_home_directory() {
    let repo = TempDir::new().unwrap();
    let kardashev_home = repo.path().join(".kardashev");
    std::fs::create_dir_all(&kardashev_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".kardashev/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&kardashev_home, id);

    let found = find_conversation_path_by_id_str(&kardashev_home, &id.to_string())
        .await
        .unwrap();
