...
use kardashev_core::protocol::ExecOutputStream;
...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    let conversation_manager =
        ConversationManager::with_auth(kardashev_core::CodexAuth::from_api_key("dummy"));
    let NewConversation {
        conversation: codex,
        ..
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let kardashev_home = TempDir::new().unwrap();
    let config = load_default_config_for_test(&kardashev_home);
    let conversation_manager =
        ConversationManager::with_auth(kardashev_core::CodexAuth::from_api_key("dummy"));
    let NewConversation {
        conversation: codex,
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:KARDASHEV_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${KARDASHEV_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.kardashev, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.kardashev, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.kardashev, |ev| match ev {
    ...
    let _ = wait_for_event(&test.kardashev, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
