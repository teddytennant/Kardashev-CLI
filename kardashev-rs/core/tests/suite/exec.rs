...
use kardashev_core::exec::ExecToolCallOutput;
use kardashev_core::exec::SandboxType;
use kardashev_core::exec::process_exec_tool_call;
use kardashev_core::protocol::SandboxPolicy;
use kardashev_core::spawn::KARDASHEV_SANDBOX_ENV_VAR;
use tempfile::TempDir;

use kardashev_core::error::Result;

use kardashev_core::get_platform_sandbox;

fn skip_test() -> bool {
    if std::env::var(KARDASHEV_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{KARDASHEV_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
