...
use kardashev_core::protocol::GitInfo;
...
async fn chat_mode_stream_cli() {
    ...
    cmd.arg("exec")
        ...
        .arg("hello?");
    cmd.env("KARDASHEV_HOME", home.path())
...
/// 2. Configures codex to read from this fixture via KARDASHEV_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("KARDASHEV_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("KARDASHEV_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("KARDASHEV_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("KARDASHEV_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume kardashev-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = kardashev_core::git_info::collect_git_info(&git_repo).await;
