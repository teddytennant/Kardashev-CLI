...
use kardashev_core::protocol::ItemCompletedEvent;
use kardashev_core::protocol::ItemStartedEvent;
...
async fn assistant_message_item_is_emitted() -> anyhow::Result<()> {
    ...
    let Some(kardashev_protocol::items::AgentMessageContent::Text { text }) = completed.content.first()
    else {
