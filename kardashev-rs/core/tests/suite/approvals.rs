...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.kardashev, |event| {
        matches!(
            event,
...
async fn wait_for_completion(test: &TestCodex) {
    wait_for_event(&test.kardashev, |event| {
...
async fn approving_execpolicy_amendment_persists_policy_and_skips_future_prompts() -> Result<()> {
    ...
    let policy_path = test.home.path().join("policy").join("default.kardashevpolicy");
