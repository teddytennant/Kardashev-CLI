...
use kardashev_core::ContentItem;
use kardashev_core::ConversationManager;
use kardashev_core::LocalShellAction;
use kardashev_core::LocalShellExecAction;
use kardashev_core::LocalShellStatus;
use kardashev_core::ModelClient;
use kardashev_core::ModelProviderInfo;
use kardashev_core::NewConversation;
use kardashev_core::Prompt;
use kardashev_core::ResponseEvent;
use kardashev_core::ResponseItem;
use kardashev_core::WireApi;
use kardashev_core::auth::AuthCredentialsStoreMode;
use kardashev_core::built_in_model_providers;
use kardashev_core::error::CodexErr;
...
use kardashev_protocol::models::ReasoningItemContent;
use kardashev_protocol::models::ReasoningItemReasoningSummary;
use kardashev_protocol::models::WebSearchAction;
...
/// Writes an `auth.json` into the provided `kardashev_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        kardashev_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = kardashev_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![kardashev_protocol::models::ContentItem::InputText {
    ...
    let prior_system = kardashev_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![kardashev_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = kardashev_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let kardashev_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&kardashev_home);
    ...
    let auth_manager =
        kardashev_core::AuthManager::from_auth_for_testing(CodexAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: codex,
        session_configured,
        ..
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "kardashev_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut config = load_default_config_for_test(&kardashev_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_kardashev_auth());
    let NewConversation {
        conversation: codex,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match CodexAuth::from_auth_storage(kardashev_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => kardashev_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No CodexAuth found in kardashev_home"),
...
async fn skills_append_to_instructions_when_feature_enabled() {
    ...
    let kardashev_home = TempDir::new().unwrap();
    let skill_dir = kardashev_home.path().join("skills/demo");
    ...
    config.cwd = kardashev_home.path().to_path_buf();
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let client = ModelClient::new(
        ...
        summary,
        conversation_id,
        kardashev_protocol::protocol::SessionSource::Exec,
    );
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let kardashev_fixture = builder.build(&server).await?;
    let codex = kardashev_fixture.kardashev.clone();
...
fn create_dummy_kardashev_auth() -> CodexAuth {
