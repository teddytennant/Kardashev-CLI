...
impl TestCodexBuilder {
    ...
    async fn build_with_home(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = kardashev_core::AuthManager::from_auth_for_testing(auth);
    ...
    async fn prepare_config(
        ...
        if let Ok(cmd) = assert_cmd::Command::cargo_bin("codex") {
            config.kardashev_linux_sandbox_exe = Some(PathBuf::from(cmd.get_program().to_os_string()));
        }
...
impl TestCodex {
    ...
    pub async fn submit_turn_with_policies(
        ...
        wait_for_event(&self.kardashev, |event| {
