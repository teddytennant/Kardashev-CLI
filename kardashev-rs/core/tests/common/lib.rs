...
/// avoids clobbering a developer’s real `~/.kardashev`.
pub fn load_default_config_for_test(kardashev_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        kardashev_linux_sandbox_exe: Some(cargo_bin("codex-linux-sandbox")),
...
pub async fn wait_for_event<F>(
    ...
    predicate: F,
) -> kardashev_core::protocol::EventMsg
where
    F: FnMut(&kardashev_core::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&kardashev_core::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    kardashev_core::spawn::KARDASHEV_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    kardashev_core::spawn::KARDASHEV_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    kardashev_core::shell::default_user_shell().derive_exec_args(command, true)
}
