...
use chrono::Local;
use chrono::Utc;
use kardashev_async_utils::CancelErr;
...
use kardashev_protocol::protocol::RateLimitSnapshot;
...
pub enum CodexErr {
    ...
    #[error(
        "To use Kardashev, upgrade to Plus: https://kardashev.ai/pricing."
    )]
    UsageNotIncluded,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://kardashev.ai/pricing), visit your Kardashev account dashboard to purchase more credits{}",
            ...
            Some(PlanType::Known(KnownPlan::Free)) => {
                "You've hit your usage limit. Upgrade to Plus to continue using Codex (https://kardashev.ai/pricing)."
                    .to_string()
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit your Kardashev account dashboard to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl CodexErr {
    ...
    pub fn to_kardashev_protocol_error(&self) -> CodexErrorInfo {
        match self {
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            kardashev_error_info: Some(self.to_kardashev_protocol_error()),
        }
    }
...
mod tests {
    ...
    use chrono::TimeZone;
    use chrono::Utc;
    use kardashev_protocol::protocol::RateLimitWindow;
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://kardashev.ai/pricing), visit your Kardashev account dashboard to purchase more credits or try again later."
        );
    }
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit your Kardashev account dashboard to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://kardashev.ai/pricing), visit your Kardashev account dashboard to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
