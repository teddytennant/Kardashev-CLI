...
//!
//! The history is stored at `~/.kardashev/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.kardashev`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.kardashev_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.kardashev/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        let kardashev_home = TempDir::new().expect("create temp dir");
        ...
        let history_path = kardashev_home.path().join("history.jsonl");
