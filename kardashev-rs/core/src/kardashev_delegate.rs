...
use kardashev_protocol::protocol::ApplyPatchApprovalRequestEvent;
use kardashev_protocol::protocol::Event;
use kardashev_protocol::protocol::EventMsg;
use kardashev_protocol::protocol::ExecApprovalRequestEvent;
use kardashev_protocol::protocol::Op;
use kardashev_protocol::protocol::SessionSource;
use kardashev_protocol::protocol::SubAgentSource;
use kardashev_protocol::protocol::Submission;
...
pub(crate) async fn run_kardashev_conversation_interactive(
    config: Config,
    ...
    let kardashev_for_events = Arc::clone(&codex);
    tokio::spawn(async move {
        forward_events(
            kardashev_for_events,
            tx_sub,
    ...
    let kardashev_for_ops = Arc::clone(&codex);
    tokio::spawn(async move {
        forward_ops(kardashev_for_ops, rx_ops, cancel_token_ops).await;
    });
...
pub(crate) async fn run_kardashev_conversation_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: Arc<ModelsManager>,
    input: Vec<UserInput>,
    ...
    let io = run_kardashev_conversation_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> kardashev_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = kardashev_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, kardashev_protocol::protocol::ReviewDecision::Abort)
                .await;
            kardashev_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    use kardashev_protocol::protocol::TurnAbortedEvent;
