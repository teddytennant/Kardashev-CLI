...
use kardashev_protocol::approvals::ExecPolicyAmendment;
use kardashev_protocol::items::TurnItem;
use kardashev_protocol::protocol::FileChange;
use kardashev_protocol::protocol::HasLegacyEvent;
use kardashev_protocol::protocol::ItemCompletedEvent;
use kardashev_protocol::protocol::ItemStartedEvent;
use kardashev_protocol::protocol::RawResponseItemEvent;
use kardashev_protocol::protocol::ReviewRequest;
use kardashev_protocol::protocol::RolloutItem;
use kardashev_protocol::protocol::SessionSource;
use kardashev_protocol::protocol::TaskStartedEvent;
use kardashev_protocol::protocol::TurnAbortReason;
use kardashev_protocol::protocol::TurnContextItem;
use kardashev_rmcp_client::ElicitationResponse;
...
use kardashev_async_utils::OrCancelExt;
use kardashev_execpolicy::Policy as ExecPolicy;
use kardashev_otel::otel_event_manager::OtelEventManager;
use kardashev_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use kardashev_protocol::models::ContentItem;
use kardashev_protocol::models::ResponseInputItem;
use kardashev_protocol::models::ResponseItem;
use kardashev_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use kardashev_protocol::protocol::CodexErrorInfo;
use kardashev_protocol::protocol::InitialHistory;
use kardashev_protocol::user_input::UserInput;
use kardashev_utils_readiness::Readiness;
use kardashev_utils_readiness::ReadinessFlag;
...
impl Codex {
    ...
    pub async fn spawn(
        ...
        let exec_policy = load_exec_policy_for_features(&config.features, &config.kardashev_home)
            .await
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.kardashev_home)
        })?;
...
pub(crate) struct TurnContext {
    ...
    pub(crate) kardashev_linux_sandbox_exe: Option<PathBuf>,
...
pub(crate) struct SessionConfiguration {
    /// Provider identifier ("openai", "kardashev", ...).
...
impl Session {
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            kardashev_linux_sandbox_exe: per_turn_config.kardashev_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.clone(),
            kardashev_linux_sandbox_exe: config.kardashev_linux_sandbox_exe.clone(),
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        let (kardashev_home, current_policy) = {
            let state = self.state.lock().await;
            (
                state
                    .session_configuration
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        kardashev_error: CodexErr,
    ) {
        let kardashev_error_info = CodexErrorInfo::ResponseStreamDisconnected {
            http_status_code: kardashev_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            kardashev_error_info: Some(kardashev_error_info),
        });
        self.send_event(turn_context, event).await;
    }
...
mod handlers {
    ...
    use kardashev_protocol::custom_prompts::CustomPrompt;
    use kardashev_protocol::protocol::CodexErrorInfo;
    use kardashev_protocol::protocol::ErrorEvent;
    use kardashev_protocol::protocol::Event;
    use kardashev_protocol::protocol::EventMsg;
    use kardashev_protocol::protocol::ListCustomPromptsResponseEvent;
    use kardashev_protocol::protocol::Op;
    use kardashev_protocol::protocol::ReviewDecision;
    use kardashev_protocol::protocol::ReviewRequest;
    use kardashev_protocol::protocol::TurnAbortReason;
    use kardashev_protocol::protocol::WarningEvent;

    use kardashev_protocol::user_input::UserInput;
    use kardashev_rmcp_client::ElicitationAction;
    use kardashev_rmcp_client::ElicitationResponse;
    use mcp_types::RequestId;
    use std::sync::Arc;
    use tracing::info;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: kardashev_protocol::approvals::ElicitationAction,
    ) {
        let action = match decision {
            kardashev_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            kardashev_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            kardashev_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| kardashev_protocol::message_history::HistoryEntry {
            ...
            && let Err(e) = rec.shutdown().await
        {
            ...
            let event = Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    kardashev_error_info: Some(CodexErrorInfo::Other),
                }),
            };
            sess.send_event_raw(event).await;
        }
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        kardashev_linux_sandbox_exe: parent_turn_context.kardashev_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
async fn try_run_turn(
    ...
    let outcome: CodexResult<TurnRunResult> = loop {
        let event = match stream.next().or_cancel(&cancellation_token).await {
            Ok(event) => event,
            Err(kardashev_async_utils::CancelErr::Cancelled) => break Err(CodexErr::TurnAborted),
        };
...
mod tests {
    ...
    use kardashev_protocol::models::FunctionCallOutputPayload;
    ...
    fn set_rate_limits_retains_previous_credits() {
        let kardashev_home = tempfile::tempdir().expect("create temp dir");
        let config = Config::load_from_base_config_with_overrides(
            ConfigToml::default(),
            ConfigOverrides::default(),
            kardashev_home.path().to_path_buf(),
        )
        ...
        let initial = RateLimitSnapshot {
            ...
            plan_type: Some(kardashev_protocol::account::PlanType::Plus),
        };
    ...
    fn set_rate_limits_updates_plan_type_when_present() {
        ...
        let update = RateLimitSnapshot {
            ...
            secondary: None,
            credits: None,
            plan_type: Some(kardashev_protocol::account::PlanType::Pro),
        };
        state.set_rate_limits(update.clone());
