...
use kardashev_otel::config::OtelExporter;
use kardashev_otel::config::OtelHttpProtocol;
use kardashev_otel::config::OtelSettings;
use kardashev_otel::config::OtelTlsConfig as OtelTlsSettings;
use kardashev_otel::otel_provider::OtelProvider;
use std::error::Error;
...
pub fn build_provider(
    ...
    OtelProvider::from(&OtelSettings {
        ...
        kardashev_home: config.kardashev_home.clone(),
...
/// Keeps events that originated from kardashev_otel module
pub fn kardashev_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("kardashev_otel")
}
