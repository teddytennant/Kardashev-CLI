...
pub fn apply_blocking(
    ...
    std::fs::create_dir_all(kardashev_home).with_context(|| {
        format!(
            "failed to create Codex home directory at {}",
            kardashev_home.display()
        )
    })?;
...
pub async fn apply(
    ...
    let kardashev_home = kardashev_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&kardashev_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(kardashev_home: &Path) -> Self {
        Self {
            kardashev_home: kardashev_home.to_path_buf(),
            profile: None,
            edits: Vec::new(),
        }
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.kardashev_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let contents =
            std::fs::read_to_string(kardashev_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    fn blocking_set_model_preserves_inline_table_contents() {
        ...
        std::fs::write(
            kardashev_home.join(CONFIG_TOML_FILE),
            r#"profile = "fast"
...
"#,
        ...
        let raw = std::fs::read_to_string(kardashev_home.join(CONFIG_TOML_FILE)).expect("read config");
...
"#;
    ...
    #[test]
    fn blocking_set_hide_gpt_5_1_kardashev_max_migration_prompt_preserves_table() {
        let tmp = tempdir().expect("tmpdir");
        let kardashev_home = tmp.path();
        std::fs::write(
            kardashev_home.join(CONFIG_TOML_FILE),
...
";
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !kardashev_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        contents = std::fs::read_to_string(kardashev_home.join(CONFIG_TOML_FILE)).expect("read config");
