...
use kardashev_app_server_protocol::Tools;
use kardashev_app_server_protocol::UserSavedConfig;
use kardashev_protocol::config_types::ForcedLoginMethod;
use kardashev_protocol::config_types::ReasoningSummary;
use kardashev_protocol::config_types::SandboxMode;
use kardashev_protocol::config_types::TrustLevel;
use kardashev_protocol::config_types::Verbosity;
use kardashev_protocol::openai_models::ReasoningEffort;
use kardashev_rmcp_client::OAuthCredentialsStoreMode;
use dirs::home_dir;
...
pub const KARDASHEV_DEFAULT_MODEL: &str = "nova";
const KARDASHEV_DEFAULT_REVIEW_MODEL: &str = "nebula";
...
pub struct Config {
    ...
    /// Model used specifically for review sessions. Defaults to "nebula".
    ...
    /// event.
    ///
    /// Example `~/.kardashev/config.toml` snippet:
    ///
    /// ```toml
    ...
    ///          https://github.com/openai/codex/blob/main/kardashev-rs/rmcp-client/src/oauth.rs#L2
    /// file: KARDASHEV_HOME/.credentials.json
    ...
    /// Directory containing all Kardashev state (defaults to `~/.kardashev` but can be
    /// overridden by the `KARDASHEV_HOME` environment variable).
    pub kardashev_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.kardashev/history.jsonl`.
    pub history: History,
    ...
    pub kardashev_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    pub async fn load_with_cli_overrides(
        ...
        let kardashev_home = find_kardashev_home()?;
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, kardashev_home)
    }
}
...
async fn load_resolved_config(
    ...
    let layers = load_config_layers_with_overrides(kardashev_home, overrides).await?;
...
pub async fn load_global_mcp_servers(
    ...
    let root_value = load_config_as_toml(kardashev_home).await?;
...
/// Patch `KARDASHEV_HOME/config.toml` project state to set trust level.
/// Use with caution.
pub fn set_project_trust_level(
    ...
    ConfigEditsBuilder::new(kardashev_home)
        .set_project_trust_level(project_path, trust_level)
        .apply_blocking()
}

/// Save the default OSS provider preference to config.toml
pub fn set_default_oss_provider(kardashev_home: &Path, provider: &str) -> std::io::Result<()> {
    ...
    let config_path = kardashev_home.join(CONFIG_TOML_FILE);
...
/// Base config deserialized from ~/.kardashev/config.toml.
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        cfg: ConfigToml,
        overrides: ConfigOverrides,
        kardashev_home: PathBuf,
    ) -> std::io::Result<Self> {
        let user_instructions = Self::load_instructions(Some(&kardashev_home));
    ...
    fn load_instructions(kardashev_dir: Option<&Path>) -> Option<String> {
        let base = kardashev_dir?;
...
fn default_model() -> String {
    KARDASHEV_DEFAULT_MODEL.to_string()
}

fn default_review_model() -> String {
    KARDASHEV_DEFAULT_REVIEW_MODEL.to_string()
}

/// Returns the path to the Kardashev configuration directory, which can be
...
///   directory exists.
pub fn find_kardashev_home() -> std::io::Result<PathBuf> {
...
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.kardashev_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    fn config_defaults_to_file_cli_auth_store_mode() -> std::io::Result<()> {
        let kardashev_home = TempDir::new()?;
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let kardashev_home = TempDir::new()?;
        let managed_path = kardashev_home.path().join("managed_config.toml");
        let config_path = kardashev_home.path().join(CONFIG_TOML_FILE);
        ...
        let root_value = load_resolved_config(kardashev_home.path(), Vec::new(), overrides).await?;
        let cfg: ConfigToml = root_value.try_into().map_err(|e| {
            tracing::error!("Failed to deserialize overridden config: {e}");
            std::io::Error::new(std::io::ErrorKind::InvalidData, e)
        })?;
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(kardashev_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(kardashev_home.path()).await?;
    ...
    async fn managed_config_wins_over_cli_overrides() -> anyhow::Result<()> {
        ...
        std::fs::write(
            kardashev_home.path().join(CONFIG_TOML_FILE),
            "model = \"base\"\n",
        )?;
...
"#,
        ...
        let err = load_global_mcp_servers(kardashev_home.path())
            .await
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(kardashev_home.path())
            .set_model(Some("gpt-5.1-codex"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(kardashev_home.path().join(CONFIG_TOML_FILE)).await?;
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn kardashev_home(&self) -> PathBuf {
            self.kardashev_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let kardashev_home = TempDir::new()?;
        let workspace = kardashev_home.path().join("workspace");
...
"#;
        ...
        let kardashev_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            kardashev_home: kardashev_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    ///    `crate::flags::KARDASHEV_DEFAULT_MODEL`
    ///
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        let o3_profile_config: Config = Config::load_from_base_config_with_overrides(
            ...
            fixture.kardashev_home(),
        )?;
        assert_eq!(
            Config {
                model: "o3".to_string(),
                review_model: KARDASHEV_DEFAULT_REVIEW_MODEL.to_string(),
                ...
                kardashev_home: fixture.kardashev_home(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                kardashev_linux_sandbox_exe: None,
...
"#;
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let kardashev_home = temp_dir.path();
        let config_path = kardashev_home.join(CONFIG_TOML_FILE);

        // Test setting valid provider on empty config
        set_default_oss_provider(kardashev_home, OLLAMA_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        assert!(content.contains("oss_provider = \"ollama\""));

        // Test updating existing config
        std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
        set_default_oss_provider(kardashev_home, LMSTUDIO_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        ...
        let result = set_default_oss_provider(kardashev_home, "invalid_provider");
