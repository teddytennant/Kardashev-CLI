...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".kardashev";
...
fn skill_roots(config: &Config) -> Vec<PathBuf> {
    let mut roots = vec![config.kardashev_home.join(SKILLS_DIR_NAME)];
...
mod tests {
    ...
    fn make_config(kardashev_home: &TempDir) -> Config {
    ...
    fn write_skill(kardashev_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(kardashev_home.path(), dir, name, description)
    }
    ...
    fn loads_valid_skill() {
        let kardashev_home = tempfile::tempdir().expect("tempdir");
        write_skill(&kardashev_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&kardashev_home);
    ...
    fn skips_hidden_and_invalid() {
        let kardashev_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = kardashev_home.path().join("skills/.hidden");
        ...
        let invalid_dir = kardashev_home.path().join("skills/invalid");
    ...
    fn enforces_length_limits() {
        ...
        write_skill(&kardashev_home, "too-long", "toolong", &long_desc);
        let cfg = make_config(&kardashev_home);
    ...
    fn loads_skills_from_repo_root() {
        ...
        let mut cfg = make_config(&kardashev_home);
