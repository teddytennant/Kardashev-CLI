...
#[cfg(unix)]
const KARDASHEV_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/codex/managed_config.toml";
...
pub async fn load_config_as_toml(kardashev_home: &Path) -> io::Result<TomlValue> {
    load_config_as_toml_with_overrides(kardashev_home, LoaderOverrides::default()).await
}

pub async fn load_config_layers(kardashev_home: &Path) -> io::Result<LoadedConfigLayers> {
    load_config_layers_with_overrides(kardashev_home, LoaderOverrides::default()).await
}
...
pub async fn load_config_layers_with_overrides(
    ...
    load_config_layers_internal(kardashev_home, overrides).await
}
...
async fn load_config_as_toml_with_overrides(
    ...
    let layers = load_config_layers_internal(kardashev_home, overrides).await?;
...
async fn load_config_layers_internal(
    ...
    let managed_config_path =
        managed_config_path.unwrap_or_else(|| managed_config_default_path(kardashev_home));

    let user_config_path = kardashev_home.join(CONFIG_TOML_FILE);
...
fn managed_config_default_path(kardashev_home: &Path) -> PathBuf {
    ...
    #[cfg(unix)]
    {
        ...
        PathBuf::from(KARDASHEV_MANAGED_CONFIG_SYSTEM_PATH)
    }
