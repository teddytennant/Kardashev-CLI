...
use kardashev_execpolicy::AmendError;
use kardashev_execpolicy::Decision;
use kardashev_execpolicy::Error as ExecPolicyRuleError;
use kardashev_execpolicy::Evaluation;
use kardashev_execpolicy::Policy;
use kardashev_execpolicy::PolicyParser;
use kardashev_execpolicy::RuleMatch;
use kardashev_execpolicy::blocking_append_allow_prefix_rule;
use kardashev_protocol::approvals::ExecPolicyAmendment;
use kardashev_protocol::protocol::AskForApproval;
use kardashev_protocol::protocol::SandboxPolicy;
use thiserror::Error;
use tokio::fs;
...
const DEFAULT_POLICY_FILE: &str = "default.kardashevpolicy";
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: kardashev_execpolicy::Error,
    },
}
...
pub(crate) async fn load_exec_policy_for_features(
    ...
    if !features.enabled(Feature::ExecPolicy) {
        ...
        load_exec_policy(kardashev_home).await
    }
}

pub async fn load_exec_policy(kardashev_home: &Path) -> Result<Policy, ExecPolicyError> {
    let policy_dir = kardashev_home.join(POLICY_DIR_NAME);
...
pub(crate) fn default_policy_path(kardashev_home: &Path) -> PathBuf {
    kardashev_home.join(POLICY_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
pub(crate) async fn append_execpolicy_amendment_and_update(
    ...
    let policy_path = default_policy_path(kardashev_home);
...
mod tests {
    ...
    async fn loads_policies_from_policy_subdirectory() {
        ...
        fs::write(
            policy_dir.join("deny.kardashevpolicy"),
    ...
    async fn ignores_policies_outside_policy_dir() {
        let temp_dir = tempdir().expect("create temp dir");
        fs::write(
            temp_dir.path().join("root.kardashevpolicy"),
...
"#;
        let mut parser = PolicyParser::new();
        parser
            .parse("test.kardashevpolicy", policy_src)
            .expect("parse policy");
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        let kardashev_home = tempdir().expect("create temp dir");
        let current_policy = Arc::new(RwLock::new(Policy::empty()));
        let prefix = vec!["echo".to_string(), "hello".to_string()];

        append_execpolicy_amendment_and_update(kardashev_home.path(), &current_policy, &prefix)
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(kardashev_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result =
            append_execpolicy_amendment_and_update(kardashev_home.path(), &current_policy, &[]).await;
