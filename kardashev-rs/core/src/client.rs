...
use kardashev_api::AggregateStreamExt;
use kardashev_api::ChatClient as ApiChatClient;
use kardashev_api::CompactClient as ApiCompactClient;
use kardashev_api::CompactionInput as ApiCompactionInput;
use kardashev_api::Prompt as ApiPrompt;
use kardashev_api::RequestTelemetry;
use kardashev_api::ReqwestTransport;
use kardashev_api::ResponseStream as ApiResponseStream;
use kardashev_api::ResponsesClient as ApiResponsesClient;
use kardashev_api::ResponsesOptions as ApiResponsesOptions;
use kardashev_api::SseTelemetry;
use kardashev_api::TransportError;
use kardashev_api::common::Reasoning;
use kardashev_api::create_text_param_for_request;
use kardashev_api::error::ApiError;
...
use crate::flags::KARDASHEV_RS_SSE_FIXTURE;
...
impl ModelClient {
    ...
    async fn stream_responses_api(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*KARDASHEV_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream = kardashev_api::stream_from_fixture(path, self.provider.stream_idle_timeout())
...
/// Adapts the core `Prompt` type into the `kardashev-api` payload shape.
