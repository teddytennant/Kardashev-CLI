...
use crate::KARDASHEV_APPLY_PATCH_ARG1;
...
pub struct ApplyPatchRequest {
    ...
    pub kardashev_exe: Option<PathBuf>,
}
...
impl ApplyPatchRuntime {
    ...
    fn build_command_spec(req: &ApplyPatchRequest) -> Result<CommandSpec, ToolError> {
        use std::env;
        let exe = if let Some(path) = &req.kardashev_exe {
            path.clone()
        } else {
            env::current_exe()
        ...
        Ok(CommandSpec {
            program,
            args: vec![KARDASHEV_APPLY_PATCH_ARG1.to_string(), req.patch.clone()],
            cwd: req.cwd.clone(),
