use crate::spawn::KARDASHEV_SANDBOX_ENV_VAR;
...
pub const DEFAULT_ORIGINATOR: &str = "kardashev_cli_rs";
pub const KARDASHEV_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "KARDASHEV_INTERNAL_ORIGINATOR_OVERRIDE";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(KARDASHEV_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn get_kardashev_user_agent() -> String {
...
pub fn build_reqwest_client() -> reqwest::Client {
    ...
    let ua = get_kardashev_user_agent();
...
fn is_sandboxed() -> bool {
    std::env::var(KARDASHEV_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    #[test]
    fn test_get_kardashev_user_agent() {
        let user_agent = get_kardashev_user_agent();
        assert!(user_agent.starts_with("kardashev_cli_rs/"));
    }
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        assert_eq!(originator_header.to_str().unwrap(), "kardashev_cli_rs");

        // User-Agent matches the computed Codex UA for that originator
        let expected_ua = get_kardashev_user_agent();
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "kardashev_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "kardashev_cli_rs/0.0.0 (bad_suffix)"
        );
    }
    ...
    fn test_macos() {
        ...
        let user_agent = get_kardashev_user_agent();
        let re = Regex::new(
            r"^kardashev_cli_rs/\d+\.\d+\.\d+ \(Mac OS \d+\.\d+\.\d+; (x86_64|arm64)\) (\S+)$",
        )
        .unwrap();
