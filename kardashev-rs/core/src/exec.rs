...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            kardashev_linux_sandbox_exe.as_ref(),
        )
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_kardashev_home;
    ...
    let kardashev_home = find_kardashev_home().map_err(|err| {
        CodexErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve kardashev_home: {err}"
        )))
    })?;
