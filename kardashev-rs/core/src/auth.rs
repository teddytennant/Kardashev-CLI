...
use kardashev_protocol::account::PlanType as AccountPlanType;
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "KARDASHEV_REFRESH_TOKEN_URL_OVERRIDE";
...
impl CodexAuth {
    ...
    pub fn from_auth_storage(
        ...
        load_auth(kardashev_home, false, auth_credentials_store_mode)
    }
...
pub const KARDASHEV_API_KEY_ENV_VAR: &str = "KARDASHEV_API_KEY";
...
pub fn read_kardashev_api_key_from_env() -> Option<String> {
    env::var(KARDASHEV_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `kardashev_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    kardashev_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(kardashev_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(kardashev_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    match logout(kardashev_home, auth_credentials_store_mode) {
...
fn load_auth(
    kardashev_home: &Path,
    enable_kardashev_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<CodexAuth>> {
    if enable_kardashev_api_key_env && let Some(api_key) = read_kardashev_api_key_from_env() {
...
mod tests {
    ...
    async fn refresh_without_id_token() {
        let kardashev_home = tempdir().unwrap();
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let CodexAuth {
            ...
            storage: _,
            ..
        } = super::load_auth(kardashev_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, kardashev_home: &Path) -> std::io::Result<String> {
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        let kardashev_home = tempdir().unwrap();
        login_with_api_key(kardashev_home.path(), "sk-test", AuthCredentialsStoreMode::File)
            .expect("seed api key");

        let config = build_config(kardashev_home.path(), Some(ForcedLoginMethod::Chatgpt), None);
        ...
        assert!(
            !kardashev_home.path().join("auth.json").exists(),
    ...
    async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
        ...
        let config = build_config(kardashev_home.path(), None, Some("org_mine".to_string()));
    ...
    async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
        let _guard = EnvVarGuard::set(KARDASHEV_API_KEY_ENV_VAR, "sk-env");
        let kardashev_home = tempdir().unwrap();
    ...
    fn plan_type_maps_known_plan() {
        ...
        let auth = super::load_auth(kardashev_home.path(), false, AuthCredentialsStoreMode::File)
            .expect("load auth")
...
impl AuthManager {
    ...
    pub fn from_auth_for_testing(auth: CodexAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            kardashev_home: PathBuf::new(),
            inner: RwLock::new(cached),
            enable_kardashev_api_key_env: false,
    ...
    pub fn reload(&self) -> bool {
        let new_auth = load_auth(
            &self.kardashev_home,
            self.enable_kardashev_api_key_env,
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout(&self.kardashev_home, self.auth_credentials_store_mode)?;
