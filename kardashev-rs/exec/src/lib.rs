...
use kardashev_core::git_info::get_git_repo_root;
...
use kardashev_protocol::approvals::ElicitationAction;
...
use kardashev_core::default_client::set_default_originator;
...
pub async fn run_main(cli: Cli, kardashev_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("kardashev_exec".to_string()) {
    ...
    let config_toml = {
        let kardashev_home = match find_kardashev_home() {
            Ok(kardashev_home) => kardashev_home,
            Err(err) => {
    ...
    let (initial_operation, prompt_summary) = match (command, prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = kardashev_core::review_prompts::user_facing_hint(&review_request.target);
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match kardashev_core::RolloutRecorder::list_conversations(
        ...
        let path = find_conversation_path_by_id_str(&config.kardashev_home, id_str).await?;
        Ok(path)
    } else {
        Ok(None)
    }
