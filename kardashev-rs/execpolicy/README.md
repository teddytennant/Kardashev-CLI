...
## CLI
- From the Kardashev CLI, run `codex execpolicy check` subcommand with one or more policy files (for example `src/default.kardashevpolicy`) to check a command:
```bash
codex execpolicy check --policy path/to/policy.kardashevpolicy git status
```
...
```bash
cargo run -p codex-execpolicy -- check --policy path/to/policy.kardashevpolicy git status
```
- Example outcomes:
