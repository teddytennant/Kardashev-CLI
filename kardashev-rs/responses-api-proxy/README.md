...
- Authentication is fixed to `Authorization: Bearer <key>` to match the Kardashev CLI expectations.
...
- We leverage [`kardashev_process_hardening`](https://github.com/openai/codex/blob/main/kardashev-rs/process-hardening/README.md) so `codex-responses-api-proxy` is run with standard process-hardening techniques.
