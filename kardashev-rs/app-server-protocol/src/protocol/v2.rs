...
use kardashev_protocol::approvals::SandboxCommandAssessment as CoreSandboxCommandAssessment;
use kardashev_protocol::config_types::ReasoningSummary;
use kardashev_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use kardashev_protocol::items::TurnItem as CoreTurnItem;
use kardashev_protocol::models::ResponseItem;
use kardashev_protocol::openai_models::ReasoningEffort;
use kardashev_protocol::parse_command::ParsedCommand as CoreParsedCommand;
use kardashev_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
use kardashev_protocol::plan_tool::StepStatus as CorePlanStepStatus;
use kardashev_protocol::protocol::CodexErrorInfo as CoreCodexErrorInfo;
use kardashev_protocol::protocol::CreditsSnapshot as CoreCreditsSnapshot;
use kardashev_protocol::protocol::RateLimitSnapshot as CoreRateLimitSnapshot;
use kardashev_protocol::protocol::RateLimitWindow as CoreRateLimitWindow;
use kardashev_protocol::protocol::SessionSource as CoreSessionSource;
use kardashev_protocol::protocol::TokenUsage as CoreTokenUsage;
use kardashev_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
use kardashev_protocol::user_input::UserInput as CoreUserInput;
...
v2_enum_from_core!(
    pub enum AskForApproval from kardashev_protocol::protocol::AskForApproval {
        UnlessTrusted, OnFailure, OnRequest, Never
    }
);

v2_enum_from_core!(
    pub enum SandboxMode from kardashev_protocol::config_types::SandboxMode {
        ReadOnly, WorkspaceWrite, DangerFullAccess
    }
);

v2_enum_from_core!(
    pub enum ReviewDelivery from kardashev_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from kardashev_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
v2_enum_from_core!(
    pub enum CommandRiskLevel from kardashev_protocol::approvals::SandboxRiskLevel {
        Low,
        Medium,
        High
    }
...
impl SandboxPolicy {
    pub fn to_core(&self) -> kardashev_protocol::protocol::SandboxPolicy {
        match self {
            SandboxPolicy::DangerFullAccess => {
                kardashev_protocol::protocol::SandboxPolicy::DangerFullAccess
            }
            SandboxPolicy::ReadOnly => kardashev_protocol::protocol::SandboxPolicy::ReadOnly,
            SandboxPolicy::WorkspaceWrite {
                writable_roots,
                network_access,
                exclude_tmpdir_env_var,
                exclude_slash_tmp,
            } => kardashev_protocol::protocol::SandboxPolicy::WorkspaceWrite {
...
impl From<kardashev_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: kardashev_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            kardashev_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            kardashev_protocol::protocol::SandboxPolicy::ReadOnly => SandboxPolicy::ReadOnly,
            kardashev_protocol::protocol::SandboxPolicy::WorkspaceWrite {
                writable_roots,
                network_access,
                exclude_tmpdir_env_var,
...
pub struct TurnError {
    pub message: String,
    pub kardashev_error_info: Option<CodexErrorInfo>,
}
...
mod tests {
    use super::*;
    use kardashev_protocol::items::AgentMessageContent;
    use kardashev_protocol::items::AgentMessageItem;
    use kardashev_protocol::items::ReasoningItem;
    use kardashev_protocol::items::TurnItem;
    use kardashev_protocol::items::UserMessageItem;
    use kardashev_protocol::items::WebSearchItem;
    use kardashev_protocol::user_input::UserInput as CoreUserInput;
    ...
    #[test]
    fn kardashev_error_info_serializes_http_status_code_in_camel_case() {
