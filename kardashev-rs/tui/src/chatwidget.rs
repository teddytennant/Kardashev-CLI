...
use kardashev_core::config::types::Notifications;
use kardashev_core::git_info::current_branch_name;
use kardashev_core::git_info::local_git_branches;
use kardashev_core::openai_models::model_family::ModelFamily;
use kardashev_core::openai_models::models_manager::ModelsManager;
use kardashev_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use kardashev_core::protocol::AgentMessageDeltaEvent;
use kardashev_core::protocol::AgentMessageEvent;
use kardashev_core::protocol::AgentReasoningDeltaEvent;
use kardashev_core::protocol::AgentReasoningEvent;
use kardashev_core::protocol::AgentReasoningRawContentDeltaEvent;
use kardashev_core::protocol::AgentReasoningRawContentEvent;
use kardashev_core::protocol::ApplyPatchApprovalRequestEvent;
use kardashev_core::protocol::BackgroundEventEvent;
use kardashev_core::protocol::CreditsSnapshot;
use kardashev_core::protocol::DeprecationNoticeEvent;
use kardashev_core::protocol::ErrorEvent;
use kardashev_core::protocol::Event;
use kardashev_core::protocol::EventMsg;
use kardashev_core::protocol::ExecApprovalRequestEvent;
use kardashev_core::protocol::ExecCommandBeginEvent;
use kardashev_core::protocol::ExecCommandEndEvent;
use kardashev_core::protocol::ExecCommandSource;
use kardashev_core::protocol::ExitedReviewModeEvent;
use kardashev_core::protocol::ListCustomPromptsResponseEvent;
use kardashev_core::protocol::McpListToolsResponseEvent;
use kardashev_core::protocol::McpStartupCompleteEvent;
use kardashev_core::protocol::McpStartupStatus;
use kardashev_core::protocol::McpStartupUpdateEvent;
use kardashev_core::protocol::McpToolCallBeginEvent;
use kardashev_core::protocol::McpToolCallEndEvent;
use kardashev_core::protocol::Op;
use kardashev_core::protocol::PatchApplyBeginEvent;
use kardashev_core::protocol::RateLimitSnapshot;
use kardashev_core::protocol::ReviewRequest;
use kardashev_core::protocol::ReviewTarget;
use kardashev_core::protocol::StreamErrorEvent;
use kardashev_core::protocol::TaskCompleteEvent;
use kardashev_core::protocol::TokenUsage;
use kardashev_core::protocol::TokenUsageInfo;
use kardashev_core::protocol::TurnAbortReason;
use kardashev_core::protocol::TurnDiffEvent;
use kardashev_core::protocol::UndoCompletedEvent;
use kardashev_core::protocol::UndoStartedEvent;
use kardashev_core::protocol::UserMessageEvent;
use kardashev_core::protocol::ViewImageToolCallEvent;
use kardashev_core::protocol::WarningEvent;
use kardashev_core::protocol::WebSearchBeginEvent;
use kardashev_core::protocol::WebSearchEndEvent;
use kardashev_core::skills::model::SkillMetadata;
use kardashev_protocol::ConversationId;
use kardashev_protocol::account::PlanType;
use kardashev_protocol::approvals::ElicitationRequestEvent;
use kardashev_protocol::parse_command::ParsedCommand;
use kardashev_protocol::user_input::UserInput;
...
use chrono::Local;
use kardashev_common::approval_presets::ApprovalPreset;
use kardashev_common::approval_presets::builtin_approval_presets;
use kardashev_core::AuthManager;
use kardashev_core::CodexAuth;
use kardashev_core::ConversationManager;
use kardashev_core::protocol::AskForApproval;
use kardashev_core::protocol::SandboxPolicy;
use kardashev_file_search::FileMatch;
use kardashev_protocol::openai_models::ModelPreset;
use kardashev_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use kardashev_protocol::plan_tool::UpdatePlanArgs;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: kardashev_feedback::CodexFeedback,
...
pub(crate) struct ChatWidget {
    ...
    kardashev_op_tx: UnboundedSender<Op>,
    ...
    feedback: kardashev_feedback::CodexFeedback,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: kardashev_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask kardashev-core to enumerate custom prompts for this session.
    ...
    fn on_exec_command_output_delta(
        &mut self,
        _ev: kardashev_core::protocol::ExecCommandOutputDeltaEvent,
    ) {
    ...
    fn on_patch_apply_end(&mut self, event: kardashev_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: kardashev_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let kardashev_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: kardashev_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(
        ...
        let kardashev_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), conversation_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<kardashev_core::CodexConversation>,
        session_configured: kardashev_core::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Logout => {
                if let Err(e) = kardashev_core::auth::logout(
            ...
            SlashCommand::TestApproval => {
                ...
                use kardashev_core::protocol::ApplyPatchApprovalRequestEvent;
                use kardashev_core::protocol::FileChange;
    ...
    pub(crate) fn handle_kardashev_event(&mut self, event: Event) {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        match msg {
            ...
            _ => {
                tracing::trace!("handle_kardashev_event: {:?}", msg);
            }
        }
    ...
    fn on_entered_review_mode(&mut self, review: ReviewRequest) {
        ...
        let hint = review
            .user_facing_hint
            .unwrap_or_else(|| kardashev_core::review_prompts::user_facing_hint(&review.target));
    ...
    fn on_exited_review_mode(&mut self, review: ExitedReviewModeEvent) {
        ...
        if let Some(output) = review.review_output {
            ...
            if output.findings.is_empty() {
                ...
                let message_text =
                    kardashev_core::review_format::format_review_findings_block(&output.findings, None);
    ...
    pub(crate) fn open_approvals_popup(&mut self) {
        ...
        for preset in presets.into_iter() {
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                {
                    if kardashev_core::get_platform_sandbox().is_none() {
                        let preset_clone = preset.clone();
                        vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match kardashev_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.kardashev_home.as_path(),
            cwd.as_path(),
            &env_map,
            &self.config.sandbox_policy,
            Some(self.config.kardashev_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn maybe_prompt_windows_sandbox_enable(&mut self) {
        if self.config.forced_auto_mode_downgraded_on_windows
            && kardashev_core::get_platform_sandbox().is_none()
    ...
    pub(crate) fn set_sandbox_policy(&mut self, policy: SandboxPolicy) {
        ...
        let should_clear_downgrade = !matches!(policy, SandboxPolicy::ReadOnly)
            || kardashev_core::get_platform_sandbox().is_some();
    ...
    pub(crate) fn submit_op(&self, op: Op) {
        ...
        if let Err(e) = self.kardashev_op_tx.send(op) {
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = kardashev_core::git_info::recent_commits(cwd, 100).await;
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<kardashev_core::git_info::CommitLogEntry>,
) {
