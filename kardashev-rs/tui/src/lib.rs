...
// The standalone `kardashev-tui` binary prints a short help message before the
...
use kardashev_common::oss::ensure_oss_provider_ready;
use kardashev_common::oss::get_default_model_for_oss_provider;
...
use kardashev_core::auth::enforce_login_restrictions;
use kardashev_core::config::Config;
use kardashev_core::config::ConfigOverrides;
use kardashev_core::config::find_kardashev_home;
use kardashev_core::config::load_config_as_toml_with_cli_overrides;
use kardashev_core::config::resolve_oss_provider;
use kardashev_core::find_conversation_path_by_id_str;
use kardashev_core::get_platform_sandbox;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = kardashev_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let kardashev_home = match find_kardashev_home() {
        Ok(kardashev_home) => kardashev_home.to_path_buf(),
        Err(err) => {
    ...
    #[allow(clippy::print_stderr)]
    let config_toml =
        match load_config_as_toml_with_cli_overrides(&kardashev_home, cli_kv_overrides.clone()).await {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&kardashev_home).await?;
    ...
    let log_dir = kardashev_core::config::log_dir(&config)?;
    ...
    // Kardashev CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("kardashev-tui.log"))?;
    ...
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("kardashev_core=info,kardashev_tui=info,kardashev_rmcp_client=info")
        })
    };
    ...
    let feedback = kardashev_feedback::CodexFeedback::new();
    ...
    let otel = kardashev_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"));
    ...
    if let Some(provider) = otel.as_ref() {
        let otel_layer = OpenTelemetryTracingBridge::new(&provider.logger).with_filter(
            tracing_subscriber::filter::filter_fn(kardashev_core::otel_init::kardashev_export_filter),
        );
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: kardashev_core::protocol::TokenUsage::default(),
    ...
    let auth_manager = AuthManager::shared(
        initial_config.kardashev_home.clone(),
        false,
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.kardashev_home, id_str).await? {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let kardashev_home = config.kardashev_home.clone();
        match CodexAuth::from_auth_storage(&kardashev_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use kardashev_core::config::ProjectConfig;
