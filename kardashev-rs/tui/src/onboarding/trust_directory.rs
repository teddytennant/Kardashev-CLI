...
use kardashev_core::config::set_project_trust_level;
use kardashev_core::git_info::resolve_root_git_project_for_trust;
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trust_level(&self.kardashev_home, &target, TrustLevel::Trusted) {
    ...
    fn handle_dont_trust(&mut self) {
        ...
        let target =
            resolve_root_git_project_for_trust(&self.cwd).unwrap_or_else(|| self.cwd.clone());
        if let Err(e) = set_project_trust_level(&self.kardashev_home, &target, TrustLevel::Untrusted) {
...
mod tests {
    ...
    fn release_event_does_not_change_selection() {
        let kardashev_home = TempDir::new().expect("temp home");
        let mut widget = TrustDirectoryWidget {
            kardashev_home: kardashev_home.path().to_path_buf(),
            cwd: PathBuf::from("."),
            is_git_repo: false,
            selection: None,
