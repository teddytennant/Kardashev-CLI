...
pub enum UpdateAction {
    /// Update via `npm install -g @kardashev/cli@latest`.
    NpmGlobalLatest,
    /// Update via `bun install -g @kardashev/cli@latest`.
    BunGlobalLatest,
...
impl UpdateAction {
    ...
    pub fn command_args(self) -> (&'static str, &'static [&'static str]) {
        match self {
            UpdateAction::NpmGlobalLatest => ("npm", &["install", "-g", "@kardashev/cli"]),
            UpdateAction::BunGlobalLatest => ("bun", &["install", "-g", "@kardashev/cli"]),
...
pub(crate) fn get_update_action() -> Option<UpdateAction> {
    ...
    let managed_by_npm = std::env::var_os("KARDASHEV_MANAGED_BY_NPM").is_some();
    let managed_by_bun = std::env::var_os("KARDASHEV_MANAGED_BY_BUN").is_some();
