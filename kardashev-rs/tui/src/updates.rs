...
use kardashev_core::default_client::create_client;
...
pub fn get_upgrade_version(config: &Config) -> Option<String> {
    ...
    info.and_then(|info| {
        if is_newer(&info.latest_version, KARDASHEV_CLI_VERSION).unwrap_or(false) {
...
fn version_filepath(config: &Config) -> PathBuf {
    config.kardashev_home.join(VERSION_FILENAME)
}
