...
use kardashev_core::protocol::ExecPolicyAmendment;
...
use kardashev_core::protocol::ReviewCodeLocation;
use kardashev_core::protocol::ReviewFinding;
use kardashev_core::protocol::ReviewLineRange;
use kardashev_core::protocol::ReviewOutputEvent;
...
use kardashev_core::protocol::TaskStartedEvent;
...
fn set_windows_sandbox_enabled(enabled: bool) {
    kardashev_core::set_windows_sandbox_enabled(enabled);
}
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = kardashev_core::protocol::SessionConfiguredEvent {
    ...
    chat.handle_kardashev_event(Event {
        id: "initial".into(),
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let init = ChatWidgetInit {
        ...
        feedback: kardashev_feedback::CodexFeedback::new(),
        skills: None,
        is_first_run: true,
        model_family,
    };
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = kardashev_core::parse_command::parse_command(&command);
...
fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        kardashev_core::git_info::CommitLogEntry {
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_kardashev_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(kardashev_core::protocol::TurnAbortedEvent {
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert_matches!(decision, kardashev_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto kardashev_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
