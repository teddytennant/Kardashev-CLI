...
pub fn cap_sid_file(kardashev_home: &Path) -> PathBuf {
    kardashev_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(kardashev_home: &Path) -> CapSids {
    let path = cap_sid_file(kardashev_home);
    if path.exists() {
