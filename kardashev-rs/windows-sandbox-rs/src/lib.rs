...
mod windows_impl {
    ...
    fn ensure_kardashev_home_exists(p: &Path) -> Result<()> {
    ...
    pub fn run_windows_sandbox_capture(
        ...
        ensure_kardashev_home_exists(kardashev_home)?;

        let current_dir = cwd.to_path_buf();
        let logs_base_dir = Some(kardashev_home);
        log_start(&command, logs_base_dir);
        let cap_sid_path = cap_sid_file(kardashev_home);
        let is_workspace_write = matches!(&policy, SandboxPolicy::WorkspaceWrite { .. });

        let (h_token, psid_to_use): (HANDLE, *mut c_void) = unsafe {
            match &policy {
                SandboxPolicy::ReadOnly => {
                    let caps = load_or_create_cap_sids(kardashev_home);
