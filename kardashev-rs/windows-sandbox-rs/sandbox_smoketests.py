...
def _resolve_kardashev_cmd() -> List[str]:
    """Resolve the Codex CLI to invoke `kardashev sandbox windows`.
    ...
    candidates = [
        ws_root / "target" / "debug" / "kardashev.exe",
        ws_root / "target" / "release" / "kardashev.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "kardashev.exe",
            cargo_base / "release" / "kardashev.exe",
        ])
    ...
    if shutil.which("kardashev"):
        return ["kardashev"]

    raise FileNotFoundError(
        "Codex CLI not found. Build it first, e.g.\n"
        "  cargo build -p kardashev-cli --release\n"
        "or for debug:\n"
        "  cargo build -p kardashev-cli\n"
    )

CODEX_CMD = _resolve_kardashev_cmd()
print(CODEX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to kardashev CLI flags
...
def main() -> int:
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\kardashev_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.kardashev artifacts) denied
    kardashev_home = Path(os.environ["USERPROFILE"]) / ".kardashev"
    cap_sid_target = kardashev_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .kardashev\\policy.json"], WS_ROOT)
    add("WS: .kardashev cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .kardashev policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > kardashev_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://kardashev-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
