...
//!
//! If the keyring is not available or fails, we fall back to KARDASHEV_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use crate::find_kardashev_home::find_kardashev_home;
...
pub enum OAuthCredentialsStoreMode {
    ...
    #[default]
    Auto,
    /// KARDASHEV_HOME/.credentials.json
...
fn fallback_file_path() -> Result<PathBuf> {
    let mut path = find_kardashev_home()?;
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create KARDASHEV_HOME temp dir");
            unsafe {
                std::env::set_var("KARDASHEV_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("KARDASHEV_HOME");
            }
        }
    }
