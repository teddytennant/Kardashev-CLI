...
use kardashev_core::config::load_global_mcp_servers;
...
use kardashev_core::mcp::auth::compute_auth_statuses;
use kardashev_core::protocol::McpAuthStatus;
use kardashev_rmcp_client::delete_oauth_tokens;
use kardashev_rmcp_client::perform_oauth_login;
use kardashev_rmcp_client::supports_oauth_login;
...
/// - `add`    — add a server launcher entry to `~/.kardashev/config.toml`
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let kardashev_home = find_kardashev_home().context("failed to resolve KARDASHEV_HOME")?;
    let mut servers = load_global_mcp_servers(&kardashev_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", kardashev_home.display()))?;
    ...
    ConfigEditsBuilder::new(&kardashev_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", kardashev_home.display()))?;
