...
use kardashev_core::parse_command;
...
use kardashev_mcp_server::CodexToolCallParam;
use kardashev_mcp_server::ExecApprovalElicitRequestParams;
use kardashev_mcp_server::ExecApprovalResponse;
use kardashev_mcp_server::PatchApprovalElicitRequestParams;
use kardashev_mcp_server::PatchApprovalResponse;
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // as an elicitation.
    let kardashev_request_id = mcp_process
        .send_kardashev_tool_call(CodexToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        kardashev_request_id.to_string(),
        params.kardashev_event_id.clone(),
    )?;
    ...
    let kardashev_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(kardashev_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(kardashev_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    kardashev_mcp_tool_call_id: String,
    kardashev_event_id: String,
    ...
    let kardashev_parsed_cmd = parse_command::parse_command(&command);
    Ok(JSONRPCRequest {
        jsonrpc: JSONRPC_VERSION.into(),
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            kardashev_elicitation: "exec-approval".to_string(),
            kardashev_mcp_tool_call_id,
            kardashev_event_id,
            kardashev_command: command,
            kardashev_cwd: workdir.to_path_buf(),
            kardashev_call_id: "call1234".to_string(),
            kardashev_parsed_cmd,
            kardashev_risk: None,
        })?),
    })
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_kardashev_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = kardashev_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn kardashev_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    create_config_toml(kardashev_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(kardashev_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request(
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            kardashev_elicitation: "patch-approval".to_string(),
            kardashev_mcp_tool_call_id,
            kardashev_event_id,
            kardashev_reason: reason,
            kardashev_grant_root: grant_root,
            kardashev_changes: changes,
            kardashev_call_id: "call1234".to_string(),
        })?),
    })
}
