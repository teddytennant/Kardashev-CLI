...
impl McpProcess {
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let user_agent = format!(
            "kardashev_cli_rs/0.0.0 ({} {}; {}) {} (elicitation test; 0.0.0)",
            os_info.os_type(),
            os_info.version(),
            os_info.architecture().unwrap_or("unknown"),
            kardashev_core::terminal::user_agent()
        );
        assert_eq!(
    ...
    pub async fn send_kardashev_tool_call(
        &mut self,
        params: CodexToolCallParam,
    ) -> anyhow::Result<i64> {
        let kardashev_tool_call_params = CallToolRequestParams {
            name: "codex".to_string(),
            arguments: Some(serde_json::to_value(params)?),
        };
        self.send_request(
            mcp_types::CallToolRequest::METHOD,
            Some(serde_json::to_value(kardashev_tool_call_params)?),
        )
        .await
    }
