...
pub(crate) fn create_tool_for_kardashev_tool_call_param() -> Tool {
...
impl CodexToolCallParam {
    ...
    pub async fn into_config(
    ...
    ) -> std::io::Result<(String, kardashev_core::config::Config)> {
        let Self {
            prompt,
            model,
            profile,
        ...
        // Build the `ConfigOverrides` recognized by kardashev-core.
        let overrides = kardashev_core::config::ConfigOverrides {
            model,
            review_model: None,
        ...
        let cfg =
            kardashev_core::config::Config::load_with_cli_overrides(cli_overrides, overrides).await?;
...
pub(crate) fn create_tool_for_kardashev_tool_call_reply_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
...
mod tests {
    ...
    #[test]
    fn verify_kardashev_tool_json_schema() {
        let tool = create_tool_for_kardashev_tool_call_param();
        ...
        let expected_tool_json = serde_json::json!({
          ...
          "inputSchema": {
            ...
            "properties": {
              ...
              "config": {
                "description": "Individual config settings that will override what is in KARDASHEV_HOME/config.toml.",
    ...
    #[test]
    fn verify_kardashev_tool_reply_json_schema() {
        let tool = create_tool_for_kardashev_tool_call_reply_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
