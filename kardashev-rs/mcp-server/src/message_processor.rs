...
use crate::kardashev_tool_config::CodexToolCallParam;
use crate::kardashev_tool_config::CodexToolCallReplyParam;
use crate::kardashev_tool_config::create_tool_for_kardashev_tool_call_param;
use crate::kardashev_tool_config::create_tool_for_kardashev_tool_call_reply_param;
...
use kardashev_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_kardashev_uuid: Arc<Mutex<HashMap<RequestId, ConversationId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            running_requests_id_to_kardashev_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                ...
                user_agent: Some(get_kardashev_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_kardashev_tool_call_param(),
                create_tool_for_kardashev_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            ...
            "codex-reply" => {
                self.handle_tool_call_kardashev_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_codex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.kardashev_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
        ...
        let running_requests_id_to_kardashev_uuid = self.running_requests_id_to_kardashev_uuid.clone();

        // Spawn an async task to handle the Codex session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Codex session and stream events back to the client.
            crate::kardashev_tool_runner::run_kardashev_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_kardashev_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_kardashev_session_reply(
        &self,
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_kardashev_uuid = running_requests_id_to_kardashev_uuid.clone();

            async move {
                crate::kardashev_tool_runner::run_kardashev_tool_session_reply(
                    codex,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let conversation_id = {
            let map_guard = self.running_requests_id_to_kardashev_uuid.lock().await;
        ...
        let err = kardashev_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: kardashev_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
        ...
        self.running_requests_id_to_kardashev_uuid
            .lock()
            .await
            .remove(&request_id);
    }
