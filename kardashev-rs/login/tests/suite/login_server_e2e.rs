...
use kardashev_login::ServerOptions;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = kardashev_home.clone();

    let opts = ServerOptions {
        kardashev_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: kardashev_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = kardashev_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_kardashev_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let kardashev_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn forced_chatgpt_workspace_id_mismatch_blocks_login() -> Result<()> {
    ...
    let opts = ServerOptions {
        kardashev_home: kardashev_home.clone(),
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: kardashev_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_kardashev_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        kardashev_home: first_kardashev_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: kardashev_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_kardashev_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        kardashev_home: second_kardashev_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: kardashev_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
