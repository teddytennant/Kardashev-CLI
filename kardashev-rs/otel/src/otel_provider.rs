        ...
        match &settings.exporter {
            ...
            OtelExporter::OtlpGrpc {
                ...
                let tls_config = match tls.as_ref() {
                    Some(tls) => build_grpc_tls_config(
                        ...
                        base_tls_config,
                        tls,
                        settings.kardashev_home.as_path(),
                    )?,
            ...
            OtelExporter::OtlpHttp {
                ...
                if let Some(tls) = tls.as_ref() {
                    let client = build_http_client(tls, settings.kardashev_home.as_path())?;
...
fn build_grpc_tls_config(
    ...
    if let Some(path) = tls.ca_certificate.as_ref() {
        let (pem, _) = read_bytes(kardashev_home, path)?;
        config = config.ca_certificate(TonicCertificate::from_pem(pem));
    }

    match (&tls.client_certificate, &tls.client_private_key) {
        (Some(cert_path), Some(key_path)) => {
            let (cert_pem, _) = read_bytes(kardashev_home, cert_path)?;
            let (key_pem, _) = read_bytes(kardashev_home, key_path)?;
...
fn build_http_client(
    ...
    tokio::task::block_in_place(|| build_http_client_inner(tls, kardashev_home))
}
...
fn build_http_client_inner(
    ...
    if let Some(path) = tls.ca_certificate.as_ref() {
        let (pem, location) = read_bytes(kardashev_home, path)?;
    ...
    match (&tls.client_certificate, &tls.client_private_key) {
        (Some(cert_path), Some(key_path)) => {
            let (mut cert_pem, cert_location) = read_bytes(kardashev_home, cert_path)?;
            let (key_pem, key_location) = read_bytes(kardashev_home, key_path)?;
