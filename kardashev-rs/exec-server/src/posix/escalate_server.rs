...
use kardashev_core::exec::ExecExpiration;
...
impl EscalateServer {
    ...
    pub async fn exec(
        ...
        let result = process_exec_tool_call(
            kardashev_core::exec::ExecParams {
                command: vec![
            ...
            &sandbox_state.kardashev_linux_sandbox_exe,
            None,
        )
        .await?;
        escalate_task.abort();
...
mod tests {
    ...
    async fn handle_escalate_session_respects_run_in_sandbox_decision() -> anyhow::Result<()> {
        ...
        for i in 0..10 {
            ...
            env.insert(format!("KARDASHEV_TEST_VAR{i}"), value);
        }
