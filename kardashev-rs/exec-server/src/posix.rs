...
//! from us, and we give its fd number in the KARDASHEV_ESCALATE_SOCKET environment variable.
//!
...
use kardashev_core::is_dangerous_command::command_might_be_dangerous;
...
const KARDASHEV_EXECVE_WRAPPER_EXE_NAME: &str = "codex-execve-wrapper";
...
pub async fn main_mcp_server() -> anyhow::Result<()> {
    ...
    let execve_wrapper = match cli.execve_wrapper {
        ...
        None => {
            ...
            cwd.parent()
                .map(|p| p.join(KARDASHEV_EXECVE_WRAPPER_EXE_NAME))
                .ok_or_else(|| {
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let kardashev_home = find_kardashev_home().context("failed to resolve kardashev_home for execpolicy")?;
    kardashev_core::load_exec_policy(&kardashev_home)
        .await
