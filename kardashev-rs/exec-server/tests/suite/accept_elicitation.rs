...
use anyhow::Result;
use kardashev_exec_server::ExecResult;
...
async fn accept_elicitation_for_prompt_rule() -> Result<()> {
    ...
    // $KARDASHEV_HOME with an execpolicy that prompts for `git init` commands.
...
"#,
        kardashev_home.as_ref(),
    )
    .await?;
    let transport = create_transport(kardashev_home.as_ref())?;
    ...
    let kardashev_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let kardashev_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("codex-linux-sandbox");
        let kardashev_cli = assert_cmd::Command::cargo_bin("codex")?
            .get_program()
            .to_os_string();
        let kardashev_cli_path = std::path::PathBuf::from(kardashev_cli);
        symlink(&kardashev_cli_path, &kardashev_linux_sandbox_exe)?;
        Some(kardashev_linux_sandbox_exe)
    } else {
        None
    };
    notify_readable_sandbox(&project_root_path, kardashev_linux_sandbox_exe, &service).await?;
