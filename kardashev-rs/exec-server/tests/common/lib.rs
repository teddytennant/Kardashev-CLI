...
pub fn create_transport<P>(kardashev_home: P) -> anyhow::Result<TokioChildProcess>
where
    P: AsRef<Path>,
{
    ...
    let transport =
        TokioChildProcess::new(Command::new(mcp_executable.get_program()).configure(|cmd| {
            ...
            cmd.env("KARDASHEV_HOME", kardashev_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, kardashev_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = kardashev_home.as_ref().join("policy");
    tokio::fs::create_dir_all(&policy_dir).await?;
    tokio::fs::write(policy_dir.join("default.kardashevpolicy"), policy).await?;
    Ok(())
}
