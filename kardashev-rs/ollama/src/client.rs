...
impl OllamaClient {
    ...
    async fn try_from_provider_with_base_url(base_url: &str) -> io::Result<Self> {
        let provider =
            kardashev_core::create_oss_provider_with_base_url(base_url, kardashev_core::WireApi::Chat);
        Self::try_from_provider(&provider).await
    }
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(kardashev_core::spawn::KARDASHEV_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
