...
use bytes::Bytes;
use kardashev_api::AuthProvider;
use kardashev_api::ChatClient;
use kardashev_api::Provider;
use kardashev_api::ResponsesClient;
use kardashev_api::ResponsesOptions;
use kardashev_api::WireApi;
use kardashev_client::HttpTransport;
use kardashev_client::Request;
use kardashev_client::Response;
use kardashev_client::StreamResponse;
use kardashev_client::TransportError;
...
fn provider(name: &str, wire: WireApi) -> Provider {
    Provider {
        ...
        wire,
        headers: HeaderMap::new(),
        retry: kardashev_api::provider::RetryConfig {
            max_attempts: 1,
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    let prompt = kardashev_api::Prompt {
