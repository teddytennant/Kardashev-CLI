#!/usr/bin/env python3
"""Stage and optionally package the @openai/kardashev npm module."""
...
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "kardashev-rs" / "responses-api-proxy" / "npm"
CODEX_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "kardashev": ["kardashev", "rg"],
    "kardashev-responses-api-proxy": ["kardashev-responses-api-proxy"],
    "kardashev-sdk": ["kardashev"],
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--package",
        choices=("kardashev", "kardashev-responses-api-proxy", "kardashev-sdk"),
        default="kardashev",
        help="Which npm package to stage (default: kardashev).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "kardashev":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/kardashev.js --version\n"
                    f"    node {staging_dir_str}/bin/kardashev.js --help\n\n"
                )
            elif package == "kardashev-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/kardashev-responses-api-proxy.js --help\n\n"
                )
            else:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="kardashev-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    if package == "kardashev":
        ...
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "kardashev-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "kardashev-responses-api-proxy.js")
    ...
    elif package == "kardashev-sdk":
        package_json_path = CODEX_SDK_ROOT / "package.json"
        stage_kardashev_sdk_sources(staging_dir)
    else:
...
def stage_kardashev_sdk_sources(staging_dir: Path) -> None:
    ...
    if not dist_src.exists():
        raise RuntimeError("kardashev-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="kardashev-npm-pack-") as pack_dir_str:
