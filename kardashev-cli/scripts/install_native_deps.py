...
DEFAULT_WORKFLOW_URL = "https://github.com/openai/kardashev/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. kardashev-<target>.zst)
    dest_dir: str  # directory under vendor/<target>/ where the binary is installed
    binary_basename: str  # executable name inside dest_dir (before optional .exe)


BINARY_COMPONENTS = {
    "kardashev": BinaryComponent(
        artifact_prefix="kardashev",
        dest_dir="kardashev",
        binary_basename="kardashev",
    ),
    "kardashev-responses-api-proxy": BinaryComponent(
        artifact_prefix="kardashev-responses-api-proxy",
        dest_dir="kardashev-responses-api-proxy",
        binary_basename="kardashev-responses-api-proxy",
    ),
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to 'kardashev' and 'rg'."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    kardashev_cli_root = (args.root or CODEX_CLI_ROOT).resolve()
    vendor_dir = kardashev_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or ["kardashev", "rg"]
    ...
    with tempfile.TemporaryDirectory(prefix="kardashev-native-artifacts-") as artifacts_dir_str:
