...
```shell
KARDASHEV_DOCKER_IMAGE_NAME=codex-linux-dev
docker build --platform=linux/amd64 -t "$KARDASHEV_DOCKER_IMAGE_NAME" ./.devcontainer
docker run --platform=linux/amd64 --rm -it -e CARGO_TARGET_DIR=/workspace/kardashev-rs/target-amd64 -v "$PWD":/workspace -w /workspace/kardashev-rs "$KARDASHEV_DOCKER_IMAGE_NAME"
```

Note that `/workspace/target` will contain the binaries built for your host platform, so we include `-e CARGO_TARGET_DIR=/workspace/kardashev-rs/target-amd64` in the `docker run` command so that the binaries built inside your container are written to a separate directory.
